...
export const is_overlap_timeslots = (ts1: Timeslot, ts2: Timeslot): boolean => {
    let s1: Time = ts1.start_time
    let e1: Time = ts1.end_time
    let s2: Time = ts2.start_time
    let e2: Time = ts2.end_time
    return ts1.term === ts2.term && ts1.day === ts2.day && ((e2 > e1 && s2 < e1) || (e2 <= e1 && e2 > s1))
}
...
export const is_overlap_losections = (sections: Section[]): boolean => {
    let timeSlots = sections.reduce((lots: Timeslot[], sect: Section) => lots.concat(sect.schedule), [])
...
export const overlapCells = (c1: Cell_display, c2: Cell_display) => {
    let s1: Time = c1.start
    let e1: Time = c1.end
    let s2: Time = c2.start
    let e2: Time = c2.end
