 ...
 * @returns {Section[][]}
 */
export const filterByTermStatusActivity = (sections: Section[], term: string, status: string[], mode: string[]): Section[] => {
    const filtered = sections.filter((s: Section) => s.term === term && s.activity !== 'Waiting List' && status.includes(s.status) && mode.includes(s.mode))
    return filtered
}
