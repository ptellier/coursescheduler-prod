...
test("solve (3 of each section)", () => {
  expect((solve_opti([CPSC121_101, CPSC121_102, CPSC121_103, CPSC110_101, CPSC110_102, CPSC210_101, CPSC210_102])
  .map(loSect_to_loName))).toEqual<string[][]>([
    [CPSC121_101, CPSC110_101, CPSC210_101],
  ...
  ].map(loSect_to_loName));
});

const loSect_to_loName = (loSect: ListOfSection): string[] => {
  return loSect.map((sect:Section) => sect.name);
}
